...
fn main() {
    ...
    let xsm = partition_data(num_partitions, &v);
    
    print_partition_info(&xsm);

    let mut intermediate_sums_xsm : Vec<usize> = Vec::new();
    
    for i in 0..(num_partitions){
        intermediate_sums_xsm.push(map_data(&xsm[i]));
    }

    //intermediate_sums_xsm.push(map_data(&xsm[0]));
    //intermediate_sums_xsm.push(map_data(&xsm[1]));
    //intermediate_sums_xsm.push(map_data(&xsm[2]));
    //intermediate_sums_xsm.push(map_data(&xsm[3]));

    println!("Intermediate sums = {:?}", intermediate_sums_xsm);

    let _user = "foo";
    let _user = "bar";

    let sum_xsm = reduce_data(&intermediate_sums_xsm);
 
    println!("Sum XSM = {}", sum_xsm);
...
* 
*/
fn partition_data(num_partitions: usize, vm: &Vec<usize>) -> Vec<Vec<usize>>{
    let partition_size = vm.len() / num_partitions;
    
    // Create a vector that will contain vectors of integers
    let mut xsm: Vec<Vec<usize>> = Vec::new();

    let mut remainder = vm.len() % num_partitions;

    let mut prev = 0;

    for _i in 0..num_partitions{
        let start = prev;
        let mut end = partition_size + prev;
        if remainder > 0 {
	    end += 1;
            remainder -= 1;
        }
        ...
        let mut xsub : Vec<usize> = Vec::new();
